...
class HiveService {
  ...
  static const _bxDietRoutineSchedules = 'diet_routine_schedules';
  ...
  late Box<DietRoutineSchedule> dietRoutineSchedulesBox;
  ...
  Future<void> init() async {
    ...
    // === IDs estáveis (NÃO ALTERE após dados em produção) ===
    ...
    _reg(44, DietRoutineScheduleAdapter()); // schedule da DIETA
    ...
    workoutSessionsBox    = await Hive.openBox<WorkoutSession>(_bxWorkoutSessions);
    workoutDaysBox        = await Hive.openBox<WorkoutDay>(_bxWorkoutDays);
    workoutRoutinesBox    = await Hive.openBox<WorkoutRoutine>(_bxWorkoutRoutines);
    workoutBlocksBox      = await Hive.openBox<WorkoutBlock>(_bxWorkoutBlocks);
    routineSchedulesBox   = await Hive.openBox<WorkoutRoutineSchedule>(_bxRoutineSchedules);

    workoutSessionLogsBox = await Hive.openBox<WorkoutSessionLog>(_bxWorkoutSessionLogs);
    workoutSetEntriesBox  = await Hive.openBox<WorkoutSetEntry>(_bxWorkoutSetEntries);
    ...
    dietRoutineSchedulesBox =
        await Hive.openBox<DietRoutineSchedule>(_bxDietRoutineSchedules);
  ...
  Box<T> getBox<T>(String name) {
    if (!Hive.isBoxOpen(name)) {
      throw HiveError('Box "$name" not found. Did you forget to call Hive.openBox()?');
    }
