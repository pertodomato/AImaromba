// lib/features/3_planner/presentation/controllers/planner_controller.dart
import 'dart:async';
...
class PlannerController extends ChangeNotifier {
  ...
  void _set(PlannerState s) { _state = s; notifyListeners(); }
  ...
  Future<void> generateSummaries({
    ...
    try {
      final s = await orchestrator.generateSummaries(userProfile: userProfile, userGoal: goal, answers: answers);
  ...
  Future<void> confirmAndBuild({
    required Map<String,Object?> userProfile,
    required Map<String,String> answers,
  }) async {
    _set(_state.copyWith(step: PlanUiStep.generating, message: 'Construindo treino e dieta...', progress: 0.0));
    try {
      // Treino
      await for (final ev in orchestrator.buildWorkoutPlan(userProfile: userProfile, answers: answers)) {
        _set(_state.copyWith(step: PlanUiStep.generating, message: ev.message, progress: ev.progress));
      }
      // Dieta
      await for (final ev in orchestrator.buildDietPlan(userProfile: userProfile, answers: answers)) {
        _set(_state.copyWith(step: PlanUiStep.generating, message: ev.message, progress: ev.progress));
      }
